...
private[codegen] case class BodyProcessing(rawBodyClass: String, bodyTransformation: Option[BodyTransformation]) {
  def bodyClass: String = bodyTransformation.map { case (BodyTransformation(transformedClass, _)) =>
    transformedClass
  }.getOrElse(rawBodyClass)
}

private[codegen] case class BodyTransformation(transformedClass: String, transformFn: String)
...
private[codegen] object AnnotationParsing {
  ...
  case class ParsedAnnotation(annotationDef: AnnotationDef, params: Map[String, String])
  ...
  case class ParsedAnnotations(context: String, mods: Seq[Mod]) {
    ...
    def annotationName(annot: Mod.Annot): String = annot.collect {
      case Type.Name(annotName) => annotName
    }.headOption.getOrElse {
      throw new Exception(s"Annotation $annot name not found: $context")
    }

    val parsed: Map[AnnotationDef, Map[String, String]] = mods.flatMap {
        ...
        case (map, (name, annot)) =>
          supportedAnnotationsByName.get(name).map { annotDef =>
            if(map.contains(annotDef)) {
              ...
              throw new Exception(s"Annotation with name $name is duplicated: $context")
            }
    ...
    def includesAnnotation(annotationDef: AnnotationDef): Boolean = annotationDef.isIncluded(this)

    def containsFloppyEarsMethodAnnotations: Boolean = supportedMethodAnnotations.exists(includesAnnotation)

    def containsFloppyEarsParameterAnnotations: Boolean = supportedParameterAnnotations.exists(includesAnnotation)
    ...
    val specifiesFloppyEarsInterceptable: Boolean = {
      ...
      if (hasAnnotation && mods.exists {
        ...
        case mod"implicit   " | mod"final   " | mod"sealed   " | mod"override   " | mod"lazy   " | mod"valparam   " | mod"varparam   " | mod"case   " |
             mod"+   " | mod"-   " => true
        ...
        throw new Exception(s"Invalid modifier for annotation @$Wiretap (private | protected | implicit | final | sealed | override | case | lazy | valparam | varparam | inline")
      }
      hasAnnotation
    }
    ...
    private[this] def bodyProcessing(
      ...
      transformFnParam: AnnotationParamDef): Option[BodyProcessing] = {

        val maybeBodyClass = bodyDefParam.getParsedValue(this).map(extractClass)
        maybeBodyClass.map { bodyClass =>
          val maybeTransformedClass = transformBodyParam.getParsedValue(this).map(extractClass)
          val maybeTransformedFn = transformFnParam.getParsedValue(this)
          val maybeBodyTransformation = (maybeTransformedClass, maybeTransformedFn) match {
            case (Some(transformedClass), Some(transformedFn)) =>
              Some(BodyTransformation(transformedClass, transformedFn))
            case (None, None) =>
              None
            case _ =>
              throw new Exception(s"Missing annotation parameter for $bodyDefParam: both the transformed type and the function to transform the body to that type must be specified: $context.")
          }
          BodyProcessing(bodyClass, maybeBodyTransformation)
        }
    }

    val parseRequestJson: Option[Boolean] = WiretapRequest.parseJson.getParsedValue(this).map(_.toBoolean)

    val actionName: Option[String] = Wiretap.actionName.getParsedValue(this)

    val majorVersion: Option[Int] = Wiretap.majorVersion.getParsedValue(this).map(_.toInt)

    val convertFunction: Option[String] = WiretapConvert.convert.getParsedValue(this)
  }

  private[this] case class AnnotationParser(annot: Mod.Annot, annotationDef: AnnotationDef) {
    ...
    class AnnotationParamTraverser extends Traverser {
      ...
      val paramList: mutable.Builder[(String, String), List[(String, String)]] = List.newBuilder[(String, String)]
      ...
      override def apply(tree: Tree): Unit = tree match {
        ...
        case Term.Assign(paramName, paramValue) =>
          paramList += ((paramName.syntax, paramValue.syntax.filterNot(_ == '"')))
    ...
    def parse: ParsedAnnotation = {
      ...
      val parsedParams = normalize((new AnnotationParamTraverser).paramList.result)
      ParsedAnnotation(annotationDef, parsedParams)
    }
  }

  implicit class ModSeqOps(mods: Seq[Mod]) {
    def parseAnnotations(context: String): ParsedAnnotations = ParsedAnnotations(context, mods)
  }

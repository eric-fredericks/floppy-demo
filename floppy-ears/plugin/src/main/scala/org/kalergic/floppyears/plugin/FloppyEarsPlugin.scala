...
import org.kalergic.floppyears.codegen.{RoutesFileParserAdapter, FloppyEarsSupportGenerator}
...
object FloppyEarsPlugin extends AutoPlugin {
   ...
   * @return
   */
  private[this] def generateFloppyEarsSupport(routesFilePath: String, inputs: Seq[File], outDirPath: Path, s: TaskStreams): Set[File] = {
    s.log.info("Floppy Ears Plugin: Hello! Generating Floppy Ears support code...")

    var ps: Option[PrintStream] = None
    try {
      val routes = RoutesFileParserAdapter.parse(routesFilePath)
      FloppyEarsSupportGenerator.generateInterceptors(routes, outDirPath, inputs.map(_.absolutePath))
  ...
  def getGeneratedFileList(path: Path): Set[File] = {
    path.toFile.listFiles(new java.io.FileFilter {
  ...
  override lazy val projectSettings: Seq[Setting[_]] = Seq(
    ...
    sourceGenerators in Compile += Def.task {
      ...
      val cachedGenFn: Set[File] => Set[File] = getCachedGenFunction(routesFilePath, out, outDirPath)
  ...
  def getCachedGenFunction(routesFilePath: String, out: TaskStreams, outDirPath: Path): Set[File] => Set[File] = {
    FileFunction.cached(

...
private[codegen] case class MethodParameter(parameterContext: String, param: Term.Param, mods: Seq[Mod]) {
  import AnnotationDefinitions._
  import AnnotationParsing._
  import MethodParameter._

  val parsedAnnotations: ParsedAnnotations = mods.parseAnnotations(parameterContext)
  if (parsedAnnotations.containsFloppyEarsMethodAnnotations) {
    throw new Exception(s"Controller method parameter for $parameterContext contains invalid reference to a method annotation: ${supportedMethodAnnotations}")
  }

  def isIgnored: Boolean = parsedAnnotations.specifiesIgnoredMethodParam

  def extractExpr(routeDefinedParams: Map[String, String]): String = param match {
    ...
    case Term.Param(_, name, Some(tpe), _) =>
      ...
      val getParamExpr = s"""requestParams.get("${name.syntax.toLowerCase}").toSeq.flatten"""
    ...
    case somethingElse @ _ =>
      throw new Exception(s"Unable to determine extractExpr: ${somethingElse.syntax} for $parameterContext")
  }

  private[this] def getExprFor(convertExpr: String): String = convertExpr match {

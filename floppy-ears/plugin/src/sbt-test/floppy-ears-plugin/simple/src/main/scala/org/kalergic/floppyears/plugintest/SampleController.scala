...
import org.kalergic.floppyears.plugintest.foo.{
...
object SampleInput {
  implicit val sampleInputFormat: OFormat[SampleInput] =
    Json.format[SampleInput]
}
...
trait SampleController {
  ...
  def postMethodWithRequest(
      str1: String,
      @WiretapIgnore int1: Int,
      str2: Option[String]
  ): EssentialAction
  ...
  @Wiretap(
    actionName = "PostWithTransformedRequestAndResponse",
    majorVersion = 1
  )
  @WiretapRequest(classOf[SampleInput])
  @WiretapRequestTransform(
    transformedBody = classOf[SampleData],
    transform = TransformFunctions.sampleInputToSampleData
  )
  @WiretapResponse(classOf[SampleData])
  @WiretapResponseTransform(
    transformedBody = classOf[SampleData],
    transform = TransformFunctions.sampleDataIncrement
  )

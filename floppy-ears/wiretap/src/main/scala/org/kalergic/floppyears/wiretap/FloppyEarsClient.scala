...
object FloppyEarsClient {
  def apply(
      floppyEarsBaseUrl: String
  )(implicit dispatchers: Dispatchers): FloppyEarsClient =
    new FloppyEarsClientImpl(dispatchers, floppyEarsBaseUrl)

  private[wiretap] case class SchemaRegistration(
      source: WiretapSource,
      schema: JObject
  )
  private[wiretap] case class SchemaRegistrationResponse(
      source: WiretapSource,
      majorVersion: Int,
      minorVersion: Int
  )
  private[wiretap] case class FloppyEarsEvent(
      source: WiretapSource,
      event: JObject
  )
...
class FloppyEarsClientImpl(dispatchers: Dispatchers, floppyEarsBaseUrl: String)
    extends FloppyEarsClient {

  import FloppyEarsClient._

  private[this] val logger = Logger(this.getClass)
  private[this] implicit val ec: ExecutionContext =
    dispatchers.lookup("floppy-ears")

  private[this] implicit val backend = AkkaHttpBackend()
  private[this] implicit val serialization: Serialization.type =
  ...
  override def registerSchema(
    ...
    val schemaObj =
      JsonMethods.parse(schema.toString(false)).asInstanceOf[JObject]
    executeRequest(
      basicRequest
        .post(uri"$url")
        .body(SchemaRegistration(source, schemaObj))
        .contentType(MediaType.ApplicationJson)
        .response(asJson[SchemaRegistrationResponse])
    ) { regResponse =>
      logger.info(
        s"Successfully registered ${regResponse.source} with version ${regResponse.majorVersion}.${regResponse.minorVersion}"
      )
      ()
    }
  }
  ...
  override def sendEvent(
    ...
    executeRequest(
      basicRequest
        .post(uri"$url")
        .body(FloppyEarsEvent(source, event))
        .contentType(MediaType.ApplicationJson)
        .response(asJson[SendEventResponse])
    ) { _ =>
  ...
  private[this] def executeRequest[A, B](
      req: RequestT[Identity, Either[ResponseError[Exception], A], Nothing]

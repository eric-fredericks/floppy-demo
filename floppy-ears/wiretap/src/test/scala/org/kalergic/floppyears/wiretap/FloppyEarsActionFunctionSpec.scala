...
import org.json4s._
import org.kalergic.floppyears.wiretap.FakeFloppyEarsClient.{
  SchemaRegistration,
  SendEvent
}
...
object FloppyEarsActionFunctionSpec extends SinglePlayAppForAll {
  ...
  class FloppyEarsActionFunctionFixture extends BaseController {
    ...
    lazy val actionContext: ActionContext[MyAppRequest] =
          ...
          extractUserId =>
            extractSessionId =>
              bytes => {
                ...
                val maybeResult: Option[SomePayload] =
                  Json.fromJson[SomePayload](jsValue).asOpt
                val requestBodyAsJson =
                  ...
                  .decompose(
                    MyFloppyEvent(
                      ...
                      b = requestParams
                        ...
                        .get,
                      _requestBody =
                        Json.fromJson[SomePayload](requestBodyAsJson.json).get,
...
class FloppyEarsActionFunctionSpec
  ...
  "FloppyEarsActionFunction" should "register a schema" in new FloppyEarsActionFunctionFixture {
    ...
    val toIntercept: ActionBuilder[MyAppRequest, AnyContent] =
      MyActionBuilder(controllerComponents)
    val intercepted: ActionBuilder[MyAppRequest, AnyContent] =
      toIntercept.withFloppyEars(actionContext)
    ...
    val maybeRegistration: Option[SchemaRegistration] =
      floppyContext.client._schemaRegistrations.headOption
  ...
  "invokeBlock" should "report an event if the action succeeds" in new FloppyEarsActionFunctionFixture {
    ...
    val request: FakeRequest[AnyContentAsJson] = FakeRequest(
      method = "POST",
      path = url.withQueryParam("a", "73").withQueryParam("b", "hello")
    ).withJsonBody(Json.toJson(SomePayload(72)))

    // Volatile: will be updated in a different thread
    @volatile var execThread: Option[Thread] = None

    val resultF: Future[Result] = intercepted.invokeBlock[AnyContentAsJson](
      request,
      _ => {
    ...
    eventually {
      ...
      val eventPayload: JObject = Extraction
        .decompose(
          MyFloppyEvent(
  ...
  it should "not report an event if the action fails" in new FloppyEarsActionFunctionFixture {
    ...
    val resultF: Future[Result] =
      intercepted.invokeBlock[AnyContentAsJson](request, _ => {
        Future.failed {
